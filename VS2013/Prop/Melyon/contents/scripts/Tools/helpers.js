...
function showObject(index, b) {
    ...
    setTranslate(n_1(objectsHelpers), js(getTsOption()));
...
function getTsOption() {
    return " {rot:{x:" + getv('br_r_x') + ",y:" + getv('br_r_y') + ",z:" + getv('br_r_z') + "},sca:{x:" + getv('br_s_x') + ",y:" + getv('br_s_y') + ",z:" + getv('br_s_z') + ",a:" + getv('br_s_a') + "},pos:{x:" + getv('br_t_x') + ",y:" + getv('br_t_y') + ",z:" + getv('br_t_z') + "}}";
}
        ...
        if (i.toString().indexOf("mesh_") == 0) {
            try {
                ...
                if (isShowMesh(i.replaceAll('mesh_', ''))) {
                    get('storage-helper').textContent += i + ":::sep:::" + at + ":::sepn:::";
                }
...
function __l() {
    if (!confirm('warrning')) return;
    if (get('storage-helper').textContent == "") get('storage-helper').textContent = get('storage-helper').value;
    var s = get('storage-helper').textContent.split(":::sepn:::");

    _each(s, function (at) {

        var s2 = at.split(":::sep:::");

        localStorage.setItem(s2[0], s2[1]);
